// Imports SELECTED existing markdown pages into the CMS as Ressources Article pages.
//
// A parallel, Ressources-specific twin of `importServiceArticles.js` — same design goals, same
// safety guarantees, same code shape — rather than a change to that file, because Ressources is a
// genuinely different CMS template (`ressources-article`, single section `ressources`, content
// under `content/resources/` — note the folder is spelled the English way, the template's section
// key the French way) with its own, simpler content shape: `{ badge, body }` only, no Stats/Topics/
// FAQ extraction of any kind (`RessourceArticleView` in `RessourceArticle.jsx` never runs any such
// extractor — it is Hero + one rich-text body + a purely-derived `##`-heading rail, always). Reusing
// `serviceArticleContainers.js`'s `disassembleServiceArticle` for this content would be actively
// wrong: its `usesEditorialSystem(section)` only excludes `'guides'`, so it would try to extract a
// Stats strip / FAQ / Topics list from Ressources bodies that were never written with those
// conventions in mind — a real corruption risk, not a theoretical one. So there is no "container" to
// disassemble here at all: the Admin's one Markdown field IS the stored body, verbatim.
//
...
//   • Explicit slugs only (no "import everything"), leaf pages only.
...
//
//   node src/cms/migrate/importResourcesArticles.js --slugs=cas-pratiques            (dry run)
//   node src/cms/migrate/importResourcesArticles.js --slugs=cas-pratiques --apply
...
// `ServiceCategoryDirectory` — so it can never appear in `slugs` here (no markdown file exists for
// it to be found).
const HUB_SLUGS = new Set(['guides-livres-blancs-cse'])
...
async function loadFrontendHelpers() {
  ...
  const { RESSOURCES_NAV } = await import(pathToFileURL(path.join(clientSrc, 'constants/resourcesNav.js')))
  return { parseContentFile, RESSOURCES_NAV }
}

function findMarkdown(slug, contentRoot) {
  const file = path.join(contentRoot === CONTENT_ROOT ? RESOURCES_DIR : path.join(contentRoot, 'resources'), `${slug}.md`)
...
export async function planImport(slugs, { contentRoot = CONTENT_ROOT } = {}) {
  const { parseContentFile, RESSOURCES_NAV } = await loadFrontendHelpers()
  const template = getTemplate('ressources-article')
  const section = template.sections[0].key // 'ressources' — the template's only section
  const plan = []
  ...
  for (const slug of slugs) {
    ...
    const file = findMarkdown(slug, contentRoot)
    if (!file) { item.reason = 'No markdown file found for this slug in content/resources'; continue }

    const raw = fs.readFileSync(file, 'utf8')
    const parsed = parseContentFile(raw, file, 'Ressources')
    if (!parsed.title || parsed.title === slug) { item.reason = 'Could not parse the markdown header'; continue }

    const navIndex = RESSOURCES_NAV.findIndex((n) => n.to === template.pathFor(section, slug))
    ...
    Object.assign(item, {
      ok: true,
      section,
      path: template.pathFor(section, slug),
      ...
      page: {
        ...
        slug,
        path: template.pathFor(section, slug),
        navLabel: navIndex >= 0 ? RESSOURCES_NAV[navIndex].label : '',
        order: navIndex >= 0 ? navIndex : 0,
        legacy: { source: 'markdown', file: path.relative(repoRoot, file), sourceUrl: parsed.sourceUrl || '' },
...
if (isCli) {
  ...
  if (!slugs.length) {
    console.error('Usage: importResourcesArticles.js --slugs=slug-one,slug-two [--apply]\n(Explicit slugs are required; there is no "import all".)')
    process.exit(1)
  }
